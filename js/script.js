...
'use strict';
(function () {
  window.catalogueItem = {
    render: function (data, parent) {
      const itemSection = document.createElement('section');
      itemSection.classList.add('catalogue-item');
  
      const itemImgBox = document.createElement('div');
      itemImgBox.classList.add('catalogue-item__img-box');
      ...
      itemImgBox.appendChild(itemPrice);
  
      itemSection.appendChild(itemImgBox);
  
      const itemTitle = document.createElement('h2');
      itemTitle.classList.add('catalogue-item__title');
      const itemTitleLink = document.createElement('a');
      itemTitleLink.classList.add('catalogue-item__link',  'link-on-dark');
      itemTitleLink.textContent = data.name;
      itemTitle.appendChild(itemTitleLink);
  
      itemSection.appendChild(itemImgBox);
      itemSection.appendChild(itemTitle);

      parent.appendChild(itemSection);
    },
  };
})();
...
'use strict';
(function () {
  if (document.querySelector('.catalogue-items')) {
    let iceCreamData = [];
    const items_per_page = 12;
    const catalogueItems = document.querySelector('.catalogue-items');

    const ITEMS_COUNT_PER_PAGE = 12;
    
    fetch('https://gvenya.github.io/glac/pins-data.json').then(response => response.json())
      .then(function (data)  {
        iceCreamData = data;
        console.log(iceCreamData);
        for (let i = 0; i < items_per_page; i++) {
          window.catalogueItem.render(iceCreamData[i], catalogueItems);
        }

        /*
        const totalItemsCount = iceCreamData.length;
        const pageCount = Math.ceil(totalItemsCount / ITEMS_COUNT_PER_PAGE);
...
'use strict';
(function () {
  if (document.querySelector('.catalogue-pagination__number-btns')) {
    const numberBtns = document.querySelector('.catalogue-pagination__number-btns');
    const navBtns = document.querySelectorAll('.catalogue-pagination__nav-btn');

    const disabledBtns = {
      numberBtn: {
        content: null,
        disabledClass: 'catalogue-pagination__number-btn--active',
      },
      navBtn: {
        content: null,
        disabledClass: 'catalogue-pagination__nav-btn--disabled',
      },
      
      disabledSum: 0,
      
      enableBtn: function (btnType) {
        this[btnType].content.classList.remove(this[btnType].disabledClass);
        this[btnType].content.disabled = false;
        this[btnType].content = null;

        const changeDisabledSum = () => {
          this.disabledSum -= 1;
        };
        changeDisabledSum();
      },
      ...
      enableBtns: function () {
        switch (this.disabledSum) {
          case 2:
            this.enableBtn('navBtn');
          case 1:
            this.enableBtn('numberBtn');
        }
      },

      disableBtn: function (btnNode) {
        const btnType = btnNode.dataset.type;
        btnNode.disabled = true;
        btnNode.classList.add(this[btnType].disabledClass);
        this[btnType].content = btnNode;
        const changeDisabledSum = () => {
          this.disabledSum += 1;
        };
        changeDisabledSum();
      ...
      startSetDisabledBtns: function (numberBtnNode, navBtnNode) {
        this.disableBtn(numberBtnNode);
        this.disableBtn(navBtnNode);
        this.disabledSum = 2;
      },
    };

    disabledBtns.startSetDisabledBtns(numberBtns.children[0], navBtns[0]);

    // ----*****************************-- //

    const ITEMS_COUNT_PER_PAGE = 12;
    const totalItemsCount = 39;// 18;
    const pageCount = Math.ceil(totalItemsCount / ITEMS_COUNT_PER_PAGE);
    const LEFT_BORDER = 1;
    const RIGHT_BORDER = pageCount;
    
    const bordersToNavBtnPrefixes = {
      [LEFT_BORDER]: 'prev',
      [RIGHT_BORDER]: 'next',
    };
    const bordersToNavBtns = {
      [LEFT_BORDER]: document.querySelector('.catalogue-pagination__nav-btn--prev'),
      [RIGHT_BORDER]: document.querySelector('.catalogue-pagination__nav-btn--next'),
    };
    /*
    const pageCount = window.getPageCount();
    console.log(pageCount);
    */

    // ----*****************************-- //
    let activePageId = LEFT_BORDER;// move to start function

    const disableNavBtn = function (nextActivePageId) {
      let navBtnToDisable;//O
      if (nextActivePageId === LEFT_BORDER || nextActivePageId === RIGHT_BORDER) {
        navBtnToDisable = bordersToNavBtns[nextActivePageId];
        disabledBtns.disableBtn(navBtnToDisable);
      }
    };

    const numberBtnActivateHandler = function (evt) {
      disabledBtns.enableBtns();
      
      const nextActiveBtn = evt.target;//D
      const nextActivePageId = parseInt(nextActiveBtn.textContent);
      disabledBtns.disableBtn(nextActiveBtn);

      disableNavBtn(nextActivePageId);

      activePageId = nextActivePageId;
    };

    const navBtnActivateHandler = function (evt) {
      disabledBtns.enableBtns();
      
      const thisNavBtn = evt.target;
      let nextActivePageId;
      if (thisNavBtn.classList.contains('catalogue-pagination__nav-btn--prev')) {
        nextActivePageId = activePageId - 1;
      }
      if (thisNavBtn.classList.contains('catalogue-pagination__nav-btn--next')) {
        nextActivePageId = activePageId + 1;
      }
      const nextActiveBtn = numberBtns.children[nextActivePageId - 1];
      disabledBtns.disableBtn(nextActiveBtn);
    ...
    numberBtns.addEventListener('click', numberBtnActivateHandler);

    for (let i = 0; i < navBtns.length; i++) {
      navBtns[i].addEventListener('click', navBtnActivateHandler);
    }
  } 
})();
